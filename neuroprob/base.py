...
def _expand_cov(cov):
    ...
    if len(cov.shape) != 4: # trials, out, timesteps, dims
        raise ValueError('Shape of input covariates at most trials x out x timesteps x dims')
...
class _likelihood(_data_object):
    ...
    """
    def __init__(self, tbin, F_dims, neurons, inv_link, tensor_type=torch.float, mode='MC'):
        """
...
class _input_mapping(nn.Module):
    ...
    """
    def __init__(self, input_dims, out_dims, tensor_type=torch.float, active_dims=None, MC_only=False):
        """
    ...
    def to_XZ(self, covariates, trials=1):
        ...
        for cov_ in covariates:
            ...
            if cov_.shape[1] > 1:
                if out_dims == 1:
                    ...
                    raise ValueError('Output dimensions in covariates dimensions are not consistent')
        ...
        XZ = torch.cat(cov_list, dim=-1).expand(trials, out_dims, timesteps, -1).to(self.dummy.device)
        return XZ

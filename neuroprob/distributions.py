...
class TorchDistribution(Distribution): # Pyro distribution mixin
    """
    Module to interface with PyTorch distributions. (Similar to Pyro)
...
class Tn_Normal(TorchDistribution):
    ...
    def log_prob(self, value):
        ...
        loc = cloc[..., None] + self.ewald.expand(*([1]*len(cloc.shape)), self.ewald.shape[0]) * 2*math.pi # Ewald summation
        lprob = -((value[..., None] - loc) ** 2) / (2 * var) - log_scale - 0.5*self.logTwoPi
...
class Tn_MVN(TorchDistribution): # Not scalable as ReLie sum grows exponentially
    ...
    def __init__(self, loc, covariance_matrix=None, precision_matrix=None, scale_tril=None, validate_args=None):
        ...
        batch_shape, event_shape = self.loc.shape[:-1], self.loc.shape[-1:]
        super(Torus_Normal, self).__init__(batch_shape, event_shape, validate_args=validate_args)
...
class Tn_Uniform(TorchDistribution):
    ...
    def rsample(self, sample_shape=torch.Size()):
        shape = self._extended_shape(sample_shape)
        return torch.rand(shape, dtype=self.loc.dtype, device=self.loc.device)*2*math.pi
...
class S2_VMF(TorchDistribution):
    ...
    def __init__(self, loc, scale, *args, validate_args=None):
        ...
        if not (torch.abs(loc.norm(2, -1) - 1) < 1e-6).all():
            raise ValueError('direction vectors are not normalized, {:.2e}'.format((loc.norm(2, -1)-1).max()))
    ...
    def log_prob(self, value):
        ...
        log_normalizer = concentration.log() - torch.log(1 - torch.exp(-2*concentration)) - self.logTwoPi
        return concentration*(self.loc * value).sum(-1) - concentration + log_normalizer
    ...
    def rsample(self, sample_shape=torch.Size()):
        ...
        eps = _standard_normal((*shape[:-1], 2), dtype=self.loc.dtype, device=self.loc.device)
        concentration = 1/self.scale**2
        concentration = concentration.expand(sample_shape + concentration.shape)[..., None]
        
        W = 1 + 1./concentration * torch.log(u + (1-u)*torch.exp(-2*concentration))
        V = eps / eps.norm(2, -1)[..., None]
        x_ = torch.cat((V*torch.sqrt(1-W**2), W), dim=-1)
        mu = self.loc

        R = torch.empty(*shape, shape[-1], dtype=self.scale.dtype, device=self.scale.device)
    ...
    def entropy(self, samples):
        ...
        """
        concentration = 1/self.scale**2
        return samples.new_ones(*samples.shape[1:]) * (1-concentration*(1+1/torch.tanh(concentration)) + \
                torch.log(1 - torch.exp(-2*concentration)) - torch.log(concentration) + self.logTwoPi)
...
class Sn_Uniform(TorchDistribution):
    ...
    def log_prob(self, value):
        ...
        return -torch.ones_like(value[0, :, 0], device=self.loc.device) * ((self.n/2.)*self.logPi - 
                                                                  math.lgamma(self.n/2.+1.))
    ...
    def entropy(self, samples):
        ...
        return samples.new_ones(samples.shape[1]) * ((self.n/2.)*self.logPi - math.lgamma(self.n/2.+1.))
...
class SO3_Uniform(TorchDistribution):
    ...
    def log_prob(self, value):
        ...
        return -torch.ones_like(value[..., 0], device=self.loc.device) * ((3./2.)*self.logPi - 
                                                                  math.lgamma(5/2.) - math.log(2.))
    
    def entropy(self, value):
        assert value.shape[-1] == 4
        return value.new_ones(value.shape[1]) * ((3./2.)*self.logPi - math.lgamma(5/2.) - math.log(2.))
...
class Delta(TorchDistribution):
    ...
    def __init__(self, loc, *args, validate_args=None):
        ...
        self.loc = loc # used for sample shape, type and device
        batch_shape, event_shape = self.loc.shape[:-1], self.loc.shape[-1:]
        super(Torus_Uniform, self).__init__(batch_shape, event_shape, validate_args=validate_args)

        self.max_ent = (math.log(math.pi) - torch.lgamma(2.))

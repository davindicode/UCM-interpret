...
class prior_variational_pair(base._VI_object):
    ...
    def validate(self, tsteps, trials, batch_info):
        ...
        if self.prior.topo != self.variational.topo:
            raise ValueError('Topologies in prior and variational distributions do not match')
    ...
    def sample(self, b, batch_info, samples, net_input, importance_weighted):
        ...
        v_samp, nlog_q = self.variational.sample(t_lower, t_upper, offs, samples, net_input)
        ...
        if importance_weighted:
            qlog_pq = (log_p + nlog_q).view(cov_samples, self.input_group.trials) - np.log(cov_samples)
...
class filtered_input(base._VI_object):
    ...
    def __init__(self, input_series, stimulus_filter, tensor_type=torch.float):
        ...
        self.history_len = self.filter.history_len # history excludes instantaneous part
...
class input_group(base._data_object):
        ...
        for k, in_ in enumerate(input_data):
            ...
            if isinstance(in_, torch.Tensor): # observed
                ...
                if in_.tensor_type != self.tensor_type:
                    raise ValueError('VI input object tensor type does not match input group')
    ...
    def _XZ(self, XZ, samples):
        ...
        if trials > 1: # cannot rely on broadcasting to get MCxtrials in first dimension
            _XZ = XZ[None, ...].repeat(samples, 1, 1, 1, 1).view(-1, out, bs, inpd).to(self.dummy.device)
        else:
    ...
    def sample_XZ(self, b, samples, net_input=None, importance_weighted=False):
        ...
        for k, in_ in enumerate(self.XZ):
            ...
            if isinstance(in_, torch.Tensor): # regressor variable
                cov.append(self._XZ(in_[..., batch_edge[b]:batch_edge[b+1], :], samples))
                continue
...
class VI_optimized(nn.Module):
    ...
    def set_model(self, input_group, mapping, likelihood):
        ...
        if max(self.mapping.active_dims) >= self.input_group.dims: # avoid illegal access
    ...
    def _nll(self, mean, variance, ll_samples, ll_mode, b, neuron, XZ, sum_time=False):
        ...
        _nll = _nll.view(-1, self.input_group.trials, _nll.shape[-1]) # (MC/GH sample, trials, time)
    ...
    def objective(self, b, neuron=None, beta=1.0, cov_samples=1, ll_samples=10, 
                  importance_weighted=False, ll_mode='MC', lv_input=None):
        """
        ...
        if self.mapping.MC_only: # SVI only
            ...
            F = self.mapping.sample_F(XZ, ll_samples) # directly carries likelihood samples
        else: # VI type mapping i.e. computing first and second moments
            F, F_var = self.mapping.compute_F(XZ) # mean and covariance of input mapping (samples, neurons, timesteps)
            
        KL_prior_m = self.mapping.KL_prior(importance_weighted) # prior may need quantities computed in compute_F, e.g. Luu, KTT    
        if ll_mode == 'direct':
            F_var = None
        ...
        _nll, ws = self._nll(F, F_var, ll_samples, ll_mode, b, neuron, XZ, sum_time=True)
        ...
        if bound == 'IWAE':
            ...
            tot_samp = ll_samples*cov_samples*self.input_group.trials
            lvm_lprior = lvm_lprior.view(cov_samples, self.input_group.trials).repeat(ll_samples, 1) / tot_samp
            lvm_nlq = lvm_nlq.view(cov_samples, self.input_group.trials).repeat(ll_samples, 1) / tot_samp
            ...
            model_nlog_pq = -torch.logsumexp((-_nll + log_pq + \
                                              torch.log(ws)), dim=-2).mean() * fac # mean over trials

        else:"""
        if importance_weighted: # mean is taken over the sample dimension i.e. log E_{q(f)q(z)}[...]
            nll_term = -torch.logsumexp(-_nll+torch.log(ws), dim=-2) # sum over MC/GH samples
        else:
        ...
        if enumeration:
            ...
            for k in range(maps): # enumeration
                ...
                if bound == 'PLL': # mean is taken over the sample dimension i.e. log E_{q(f)q(z)}[...]
                    nll.append(-torch.logsumexp(-_nll+torch.log(ws), dim=-2))
                else:
                    nll.append((_nll*ws).sum(-2)) # sum over MC/GH samples
    ...
    ### optimization ###
    def set_optimizers(self, optim_tuple, opt_lr_dict, special_grads=[], extra_update_args=None):
        #  extra_params=(), nat_grad=(), nat_lr=1e0, newton_grad=(), newton_lr=1e-1
        """
    ...
    def fit(self, max_epochs, margin_epochs=10, loss_margin=0.0, neuron=None, kl_anneal_func=None,
            retain_graph=False, cov_samples=1, ll_samples=1, importance_weighted=False, ll_mode='MC', 
        ...
        for epoch in iterator:
            ...
            for b in range(batches):
                ...
                def closure():
                    ...
                    loss = self.objective(b, neuron, beta=kl_anneal_func(anneal_t), 
                                          importance_weighted=importance_weighted, 
                                          cov_samples=cov_samples, ll_samples=ll_samples, 

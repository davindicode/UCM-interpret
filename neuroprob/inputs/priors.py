...
class IndNormal(_prior):
    """
    """
    def __init__(self, mu, std, topo, dims, tensor_type=torch.float, learn_mu=False, learn_std=False):
        ...
        if learn_std:
            self.register_parameter('finv_std', Parameter(self.lf_inv(std.type(self.tensor_type))))
        else:
...
class IndUniform(_prior):
    ...
    def __init__(self, topo, dims, tensor_type=torch.float):
        ...
        self.register_buffer('mu', prior[0].type(self.tensor_type)) # lower limit, used from dim
...
class AR1(ARNormal):
    ...
    Stationary AR(1)
    """
    def __init__(self, loc, std, dims, learn_loc=False, learn_std=True, tensor_type=torch.float):
        ...
        class transition_(nn.Module):
            def __init__(self, loc, std, learn_loc, learn_std, tensor_type):
                ...
                if learn_std:
                    self.register_parameter('finv_std', Parameter(self.lf_inv(std.type(tensor_type))))
                else:
...
class ARp(ARNormal):
    ...
    """
    def __init__(self, loc, std, learn_loc=False, learn_std=True, tensor_type=torch.float):
...
class dAR1(ARNormal):
    ...
    """
    def __init__(self, loc, std, topo, dims, learn_loc=False, learn_std=True, tensor_type=torch.float):

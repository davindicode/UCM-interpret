...
class IndNormal(_variational):
    """
    """
    def __init__(self, mu, std, topo, dims, amortized=False, NF=None, tensor_type=torch.float):
        ...
        if amortized:
            if isinstance(v_mu, nn.Module) is False or isinstance(v_std, nn.Module) is False:
            ...
            self.register_parameter(
                'mu', 
                Parameter(mu.type(self.tensor_type))
            ) # mean
       
            self.register_parameter(
                'finv_std', 
                Parameter(self.lf_inv(std.type(self.tensor_type)))
            ) # std
    ...
    def eval_moments(self, t_lower, t_upper, net_input):
        if self.amortized: # compute the variational parameters
            ...
            mu, std = self.mu[t_lower:t_upper, ...], self.lf(self.finv_std[t_lower:t_upper, ...]) # time, (dims, trial)
...
class Autoregressive(_variational):
    """
    """
    def __init__(self, mu, std, topo, dims, latent_f='softplus', amortized=False, NF=None, tensor_type=torch.float):
        super().__init__(tensor_type, mu.shape[0], dims)
        self.topo = topo
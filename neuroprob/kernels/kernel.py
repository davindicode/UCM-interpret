...
class inducing_points(nn.Module):
    """
    """
    def __init__(self, out_dims, inducing_points, constraints, MAP=False, tensor_type=torch.float):
        """
    ...
    def constrain(self):
        ...
        if self.u_scale_tril is not None: # constrain K to be PSD, L diagonals > 0 as needed for det K
            ...
            Nu = self.n_ind
            self.u_scale_tril.data[:, range(Nu), range(Nu)] = \
                torch.clamp(self.u_scale_tril.data[:, range(Nu), range(Nu)], min=1e-12)
...
class Kernel(nn.Module):
    ...
    def sample_spectral_density(self, samples):
        """
        Returns vector of :math:`\omega` (samples from spectral density)
        Non-stationary kernels have spectral surfaces :mathm:`k(\omega, \omega')`
        """
        raise NotImplementedError
...
class KTT_kernel(Kernel):
    """
    Generates a product kernel with TT-format
    """
    def __init__(self, K_list, track_dims_list):
        """
        General cores.
        The shape is (batch_shape, core_r, ij_index)
        Each kernel in K_list represents one dimension of the grid
        """
        if len(K_list) != len(track_dims_list):
            raise ValueError('Kernel list does not match track dimensions list')
            
        input_dims = 0
        for td in track_dims_list:
            input_dims += len(td)
        super().__init__(input_dims)
        self.K_list = torch.nn.ModuleList(K_list)
        self.track_dims_list = track_dims_list
        
    def Kronecker_Toeplitz(self, U_list):
        """
        Using tensor train format, Kronecker structure
        U is of shape (batch_shape, n_U, dims)
        MVMs with vectors of shape (batch_shape, n_U) or higher rank (batch_, r, n_U)
        """
        d = len(self.K_list)
        mat_list = []
        for dn, K in enumerate(self.K_list):
            U = U_list[dn]
            mat_list.append(linalg.sym_Toeplitz_matrix(K.forward(U, U[..., 0:1, :]).transpose(-2, -1), (-1,))) # get r_shape = (1,) from transpose
            
        return linalg.TT_matrix(mat_list, r_shape_len=1, tensor_shape_len=1)
    
    def Kronecker(self, U_list):
        d = len(self.K_list)
        mat_list = []
        for dn, K in enumerate(self.K_list):
            U = U_list[dn]
            mat_list.append(linalg.general_matrix(K.forward(U, U), (-1,))) # K, N, n_U, n_U
            
        return linalg.TT_matrix(mat_list, r_shape_len=1, tensor_shape_len=2)
    
    def X_U_list(self, X, U_list):
        """
        X is irregular input, so only Kronecker structure in the U inputs, take trace of TT-format
        """
        d = len(self.K_list)
        mat_list = []
        for dn, K in enumerate(self.K_list):
            td = self.track_dims_list[dn]
            U = U_list[dn]
            X_ = X[..., td]
            K_ = K.forward(X_, U) # K, N, n_X, n_U
            mat_list.append(linalg.general_matrix(K_[..., None, :, :], (-1,))) # add r dimension
            
        return linalg.TT_matrix(mat_list, r_shape_len=1, tensor_shape_len=2)
    
    def forward(self, X, Z=None, diag=False):
        """
        Computes iteratively all the main columns, returns vanilla matrix in TT format
        """
        if Z is None:
            Z = X
        ...
        for dn, K in enumerate(self.K_list):
            ...
            Z_ = Z[..., td]
            X_ = X[..., td]
            mat_list.append(linalg.general_matrix(K.forward(X_, Z_, diag=diag)[:, :, None, ...], (-1,))) # K, N, n_X, n_Z
...
class Product(Combination):
    ...
    def sample_spectral_density(self, samples):
        self.kern1.track_dims
        device
        omega = torch.empty((samples, self.input_dims), dtype=self.kern0.tensor_type, device=omega.device)
        return
...
class DeepKernel(Kernel):
    ...
    :type kern1: Kernel or numbers.Number
    """
    def __init__(self, input_dims, kern, mapping, track_dims=None, tensor_type=torch.float):
...
class Constant(Kernel):
    ...
    def forward(self, X, Z=None, diag=False):
        ...
        if diag:
            return self.variance[..., 0].expand(X.size(0), self._variance.shape[0], X.size(2)) # K, N, T
        else:
            return self.variance.expand(X.size(0), self._variance.shape[0], X.size(2), Z.size(2)) # K, N, T, T
...
class Polynomial(DotProduct):
    ...
    def __init__(self, input_dims, bias, degree=1, track_dims=None, f='exp', tensor_type=torch.float):
        super().__init__(input_dims, track_dims, f, tensor_type)
...
class Lengthscale(Kernel):
    """
    """
    def __init__(self, input_dims, lengthscale, w, omega, Q, track_dims=None, 
                 topology='euclid', f='exp', tensor_type=torch.float):
        super().__init__(input_dims, topology, lengthscale, track_dims, f, tensor_type)


class DSE(Lengthscale):
    r"""
    ...
    def __init__(self, input_dims, lengthscale, lengthscale_beta, beta, 
                 track_dims=None, f='exp', tensor_type=torch.float):
        super().__init__(input_dims, track_dims, f, tensor_type)
        ...
        self._lengthscale_beta = Parameter(self.lf_inv(lengthscale_beta.type(tensor_type)).t()[:, None, None, :]) # N, K, T, D
        self._lengthscale = Parameter(self.lf_inv(lengthscale.type(tensor_type)).t()) # N, D
...
def GSM(Lengthscale):
    r"""
    Implementation of General Spectral Mixture kernel. TODO

    :param torch.Tensor scale_mixture: Scale mixture (:math:`\alpha`) parameter of this
        kernel. Should have size 1.
    """
    def __init__(self, input_dims, lengthscale_net, w_net, omega_net, Q, 
                 track_dims=None, f='exp', tensor_type=torch.float):
        """
        nets are ANN mappings outputting input-dependent hyperparameters, Q per network
        """
        super().__init__(input_dims, track_dims, f, tensor_type)
        assert self.input_dims == lengthscale.shape[0] # lengthscale per input dimension
    ...
    @property
    def lengthscale(self):
        return self.lf(self._lengthscale)[:, None, None, :] # N, K, T, D
...
class Isotropy(Kernel):
    ...
    """
    def __init__(self, input_dims, topology, lengthscale, track_dims=None, f='exp', tensor_type=torch.float):
        super().__init__(input_dims, track_dims, f, tensor_type)
    ...
    def _square_scaled_dist_sphere(lengthscale, X, Z):
        ...
        Returns :math:`\|\frac{X-Z}{l}\|^2`.
        """
        return 2*((1 - (X[..., None, :]*Z[..., None, :].permute(0, 1, 3, 2, 4)).sum(-1))/(lengthscale[..., None, 0])**2)
...
class SquaredExponential(Isotropy):
    ...
    .. note:: This kernel also has name `Squared Exponential` in literature.
    """
    def __init__(self, input_dims, lengthscale, track_dims=None, 
                 topology='euclid', f='exp', tensor_type=torch.float):
        super().__init__(input_dims, topology, lengthscale, track_dims, f, tensor_type)
...
class RationalQuadratic(Isotropy):
    ...
    def __init__(self, input_dims, lengthscale, scale_mixture,
                 track_dims=None, topology='euclid', f='exp', tensor_type=torch.float):
        super().__init__(input_dims, topology, lengthscale, track_dims, f, tensor_type)

        self._scale_mixture = Parameter(self.lf_inv(scale_mixture.type(tensor_type))) # N
...
### GPFADS
class GPFADS_kernel(Kernel):
    """
    GPFADS matrix-output kernel
    """
    def forward(self):
        """
        The output matrix (N, N) is flattened into the first dimension, adhering to kernel format
        """
        return

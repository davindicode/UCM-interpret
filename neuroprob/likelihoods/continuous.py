...
class Gaussian(base._likelihood):
    ...
    def __init__(self, neurons, inv_link, log_var, tensor_type=torch.float):
        ...
        """
        super().__init__(1., neurons, neurons, inv_link, tensor_type) # dummy tbin
    ...
    def eval_dispersion_mapping(self, XZ, samples, neuron):
        ...
        return self.dispersion_mapping_f(dh).mean(0) # watch out for underflow or overflow here
    ...
    def sample(self, rate, neuron=None, XZ=None):
        ...
        if self.dispersion_mapping is None:
            ...
            log_var = self.eval_dispersion_mapping(XZ, samples, neuron)[None, ...].expand(rate.shape[0], 
                                                                                       *rate_.shape[1:])
            
        act = rate_ + np.exp(log_var/2.)*np.random.randn(rate.shape[0], len(neuron), rate.shape[-1])
        return act

...
class _count_model(base._likelihood):
    ...
    def objective(self, F_mu, F_var, XZ, b, neuron, samples=10, mode='MC'):
        ...
        if mode == 'MC':
            ...
            rates, n_l_rates, spikes = self.sample_helper(F_mu[:, neuron, :], b, neuron, samples)
            ws = torch.tensor(1./rates.shape[0])
        else:
...
class Poisson(_count_model):
    ...
    """
    def __init__(self, tbin, neurons, inv_link, tensor_type=torch.float, strict_likelihood=True):
        super().__init__(tbin, neurons, inv_link, tensor_type, strict_likelihood)
    ...
    def objective(self, F_mu, F_var, XZ, b, neuron, samples=10, mode='MC'):
        ...
        if self.inv_link == 'exp' and F_var is not None: # exact
            ...
            tfact, lfact = self.get_saved_factors(b, neuron, spikes)
            T = self.f((F_mu + F_var/2.)[:, neuron, :])*self.tbin
            nll = (-n_l_rates + T - tfact + lfact)
...
class hZI_Poisson(ZI_Poisson):
    ...
    Heteroscedastic ZIP
    """
    def __init__(self, tbin, neurons, inv_link, dispersion_mapping, tensor_type=torch.float, strict_likelihood=True):
...
class COM_Poisson(_count_model):
    ...
    """
    def __init__(self, tbin, neurons, inv_link, nu, tensor_type=torch.float, J=100, strict_likelihood=True):
        super().__init__(tbin, neurons, inv_link, tensor_type, strict_likelihood)
        if nu is not None:
        ...
        self.register_buffer('jfact', torch.lgamma(self.powers+1.))
...
class hCOM_Poisson(COM_Poisson):
    ...
    Heteroscedastic CMP
    """
    def __init__(self, tbin, neurons, inv_link, dispersion_mapping, tensor_type=torch.float, J=100, strict_likelihood=True):
        super().__init__(tbin, neurons, inv_link, None, tensor_type, J, strict_likelihood)
        self.dispersion_mapping = dispersion_mapping

...
class sigmoid_refractory(base._filter):
    ...
    """
    def __init__(self, a, tau, beta, timesteps, learnable=[True, True, True], tensor_type=torch.float):
        """
        """
...
class raised_cosine_bumps(base._filter):
    ...
    """
    def __init__(self, a, c, phi, w, timesteps, learnable=[True, True, True, True], tensor_type=torch.float):
        """
...
class hetero_raised_cosine_bumps(base._filter):
    ...
    def compute_filter(self, stim):
        ...
        t_ = torch.arange(self.filter_len, device=self.phi.device, dtype=self.tensor_type)
        t = self.filter_len-t_
        A = torch.clamp(self.a*torch.log(t+self.c)[None, None, None, :] - \
                        self.phi[..., None], min=-np.pi, max=np.pi) # (basis, post, pre, timesteps)
    ...
    def forward(self, input, stimulus):
        ...
        input_unfold = input.unfold(-1, self.filter_len, 1) # samples, neurons, timesteps, fold_dim
...
class filter_model(base._filter):
    ...
    def __init__(self, out_dim, in_dim, timesteps, tbin, filter_model, 
                 num_induc=6, tensor_type=torch.float):
        """
...
class hetero_filter_model(base._filter):
    ...
    """
    def __init__(self, out_dim, in_dim, timesteps, tbin, filter_model, 
                 inner_loop_bs=10, tensor_type=torch.float, mode='unfold'):
        """
        :param int out_dim: the number of output dimensions per input dimension (1 or neurons)
        :param int in_dim: the number of input dimensions for the overall filter (neurons)
        """
    ...
    def forward(self, input, stimulus):
        ...
        input_unfold = input.unfold(-1, self.filter_len, 1) # samples, neurons, timesteps, fold_dim
        if self.mode == 'unfold':
            stim_unfold = stimulus[:, :-1, :].unfold(1, self.filter_len, 1) # samples, timesteps, dims, fold_dim
        else: # repeat
            stim_ = stimulus[:, self.filter_len-1:, :]
            stim_unfold = stim_[..., None].expand(*stim_.shape, self.filter_len) # samples, timesteps, dims, fold_dim
...
class filtered_likelihood(base._likelihood):
    ...
    def __init__(self, likelihood, filter_obj):
        ...
        super().__init__(likelihood.tbin.item(), likelihood.F_dims, likelihood.neurons, 
                         likelihood.inv_link, likelihood.tensor_type, likelihood.mode)
    ...
    def KL_prior(self, importance_weighted=False):
        return self.filter.KL_prior(importance_weighted) + self.likelihood.KL_prior(importance_weighted)
    ...
    def set_Y(self, spikes, batch_info):
        ...
        self.likelihood.set_Y(in_spikes, batch_info) # excludes history part of spike train
        self.likelihood.all_spikes = spikes.type(self.likelihood.tensor_type) # overwrite
    ...
    def objective(self, F_mu, F_var, XZ, b, neuron, samples=10, mode='MC'):
        ...
        spk = self.all_spikes[:, neuron, batch_edge[b]:batch_edge[b+1]+self.history_len].to(self.likelihood.tbin.device)
    ...
    def sample(self, F_mu, ini_train, neuron=None, XZ=None, obs_spktrn=None):
        ...
        spikes = []
        spiketrain = torch.empty((*ini_train.shape[:2], self.history_len), 
                                 device=self.likelihood.tbin.device)
        ...
        for t in iterator:
            ...
            rate_ = self.likelihood.f(F_mu[..., t] + \

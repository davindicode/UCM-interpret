...
class Poisson_pp(base._likelihood):
    def __init__(self, tbin, neurons, inv_link, tensor_type=torch.float, allow_duplicate=True):
...
class _renewal_model(base._likelihood):
    ...
    def sample_helper(self, h, b, neuron, scale, samples):
        ...
        if self.inv_link == 'exp': # bit masking seems faster than integer indexing using spiketimes
            ...
            n_l_rates = (spikes*torch.log(rates+1e-12)).sum(-1) # rates include scaling
    ...
    def sample(self, rate, neuron=None, XZ=None):
        ...
        """
        neuron = self._validate_neuron(neuron)
        spiketimes = gen_IRP(self.ISI_dist(neuron), rate[:, neuron, :], self.tbin.item())
...
class Gamma(_renewal_model):
    ...
    def nll(self, n_l_rates, rISI, neuron):
        ...
        :rtype: torch.tensor
        """
        samples_ = n_l_rates.shape[0] # ll_samplesxcov_samples, in case of trials trial_num=cov_samples
        ...
        nll = -(shape_-1)*l_Lambda - n_l_rates + T + intervals[None, :]*torch.lgamma(shape_)
    ...
    def ISI_dist(self, n):
        ...
        return ISI_gamma(shape, scale=1./shape)
...
class log_Normal(_renewal_model):
    ...
    def ISI_dist(self, n):
        ...
        return ISI_logNormal(sigma, scale=np.exp(sigma**2/2.))

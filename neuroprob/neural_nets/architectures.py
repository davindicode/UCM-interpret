...
def SOC_W(ini_W, N_E, B=0.2, C=1.5, inhib_syn_ratio=0.4, gamma=3., 
    ...
    :parameter float inhib_syn_ratio: ratio between inhibitory synapses and total synapses
    """
    N = ini_W.shape[0]
    W = ini_W
    ...
    loc_inhib = np.where(W < 0.)
    tot_syn = N_I*N
    exist_inhib = (loc_inhib[1]-N_E) + N_I*loc_inhib[0]
    exist_inhib = np.array(random.sample(list(exist_inhib), len(exist_inhib)))
    
    tot_inhibs = int(tot_syn*inhib_syn_ratio)
    new_inhibs = int(tot_inhibs-len(loc_inhib[0]))
    if new_inhibs > 0: # number of new connections to be added
    ...
    exist_inhib = exist_inhib[:tot_inhibs] # only keep
    minloss = np.inf
    cnt = 0
    ...
    for k in iterator:
        ...
        exist_inhib = np.concatenate((exist_inhib, new_inhib)).astype(int)

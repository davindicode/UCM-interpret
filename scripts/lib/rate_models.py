...
from neuroprob import base
from neuroprob.mappings.parametric import GLM
...
class vonMises_GLM(GLM):
    ...
    def __init__(self, neurons, tensor_type=torch.float, active_dims=None):
        super().__init__(1, neurons, 3, tensor_type=tensor_type, active_dims=active_dims)
...
class Gauss_GLM(GLM):
    ...
    """
    def __init__(self, neurons, tensor_type=torch.float, active_dims=None):
        super().__init__(2, neurons, 6, tensor_type=tensor_type, active_dims=active_dims)
...
class ATI_HDC(base._input_mapping):
    """
    Angular (head direction/theta) variable GLM, with anticipatory time intervals
    """
    def __init__(self, neurons, tensor_type=torch.float, active_dims=None):
        super().__init__(2, neurons, tensor_type=tensor_type, active_dims=active_dims)
    ...
    def set_params(self, field, ATI):
        """
        :param np.array w: GLM parameters with additional ATI parameter, shape (neurons, 4)
        """
        if field is not None: # mu, prec
            self.field.data = field.type(self.tensor_type).to(self.dummy.device)
        if ATI is not None: # ATI
            self.ATI.data = ATI.type(self.tensor_type).to(self.dummy.device)
...
class PTP_1D(base._input_mapping):
    """
    ...
    def set_params(self, field, modulation, precession):
    ...
    def compute_F(self, XZ):
        ...
        theta = (XZ[..., 1] - Ap*torch.sigmoid(theta_0 + wp*x))
...
class PTP_GLM(GLM):
    ...
    def __init__(self, neurons, decoding=False, tensor_type=torch.float, active_dims=None):
        ...
        if decoding:
            super().__init__(3, neurons, 8, tensor_type=tensor_type, active_dims=active_dims)
        else:
            super().__init__(8, neurons, 8, tensor_type=tensor_type, active_dims=active_dims)
...
class PTP_field(base._input_mapping):
    """
    ...
    """
    def __init__(self, neurons, tensor_type=torch.float, active_dims=None):
        super().__init__(10, neurons, tensor_type=tensor_type, active_dims=active_dims)
    ...
    def set_params(self, mu, prec, rate_0, t_params, hd_params, dir_params, speed):
        if mu is not None:
    ...
    def compute_F(self, XZ):
        ...
        dp = (cov[..., :2] - self.mu[None, :, None, :])
        p = torch.cat((dp[..., 0:1]**2, dp[..., 1:2]**2, 2*dp[..., 0:1]*dp[..., 1:2]), dim=-1)
        
        return ((cov[..., 4:6]*self.w_t[None, :, None, :] + cov[..., 6:8]*self.w_hd[None, :, None, :] \
                + cov[..., 8:10]*self.w_dir[None, :, None, :]).sum(-1) + cov[..., 3]*self.speed[None, :, None, 0] \
                + cov[..., 2]*self.lrate[None, :, None, 0] - 0.5*(p*self.prec[None, :, None, :]).sum(-1)), 0

    
    
# Grid cell class
class grid_cells(base._input_mapping):
    """
    ...
    def __init__(self, neurons, decoding=False, tensor_type=torch.float, active_dims=None):
        ...
        self.decoding = decoding
        if decoding:
            super().__init__(3, neurons, tensor_type=tensor_type, active_dims=active_dims)
        else:
            super().__init__(5, neurons, tensor_type=tensor_type, active_dims=active_dims)
    ...
    def set_params(self, w, w_t, lrate):
        if w is not None:
            self.w.data = torch.tensor(w, device=self.dummy.device, dtype=self.tensor_type)
        if w_t is not None:
    ...
    def compute_F(self, XZ):
        ...
        return torch.exp((cov[..., 2:]*self.w_t[None, :, None, :]).sum(-1)) * h, 0
